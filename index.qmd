...
- You should know how to:
  - Install R packages
  - Use R Studio or another IDE of your choice
  - Create R scripts
  - Import data
...
mydata <- read_excel("/path/to/myfile.xlsx", sheet = "data")
```
:::
...
mydata <- read_csv("/path/to/myfile.csv", n_max = 100)
```
:::
...
bm |>
  ...
  ggplot(aes(x = mean, y = expr, fill = expr)) +
  geom_col() +
...
```{r}
#| eval: false
write_delim(mtcars, "~/Desktop/mtcars.txt")
write_delim(mtcars, "~/Desktop/mtcars.txt", delim = "---") # Don't do that!
...
arrange([\<dataframe\>]{.fragment .highlight-orange}, [\<column(s)\>]{.fragment .highlight-pastel})
...
[apply the `round()` function to all the columns between `temp` and `pressure`]{.text-outline}
...
[apply the `round()` function to **all the numerical columns**]{.text-outline}
...
- Here we want to calculate the average departure delay for each carrier.

```{r}
#| eval: false
#| code-line-numbers: "1-9|1-2|4-9"
...
<figure>
<center>
<figcaption>Source: [https://www.ifremer.fr/fr](https://www.ifremer.fr/fr)</figcaption>
<img src="https://s3-us-west-2.amazonaws.com/media.ooica.net/wp-content/uploads/2019/01/02231816/ctd_20180725_104738_sb.jpg" alt="A CTD hanging on a cable, ready to be deployed in the ocean." style="width:60%"/>
</center>
</figure>
